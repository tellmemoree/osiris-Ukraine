  ...
  useEffect(() => {
    ...
    map.on('click', 'thermal-aoi-dots', e => {
      ...
      try {
        ...
        if (srcs.length > 0) {
          const first = srcs[0];
          // For site markers that received merged news sources, show the best-extracted
          ...
          if (!isNews && (first.snippet || first.title)) {
            ...
            snippetHtml = `<div style="font-size:9px;color:#8A8880;line-height:1.4;margin-bottom:6px;font-style:italic;">"${text.slice(0, 120)}"</div>`;
          }
          const linkRows = srcs.filter((s: any) => s.link || s.source).map((s: any) => {
            const label = (s.snippet || s.title) as string | undefined;
            return `<div style="margin-bottom:3px;line-height:1.4;">${s.link ? `<a href="${s.link}" target="_blank" style="color:#D4AF37;text-decoration:none;">${s.source||'source'}</a>` : `<span style="color:#D4AF37;">${s.source}</span>`}${label ? `<span style="color:#666;font-size:8px;"> — ${label.slice(0,90)}</span>` : ''}</div>`;
          }).join('');
