/**
 * OSIRIS — MMSI → flag state resolver.
 * The first three digits of an AIS MMSI are the Maritime Identification Digits
 * (MID), assigned by the ITU to a flag state. This maps MID → ISO-3166 alpha-2,
 * and derives a flag emoji from that. Covers the full ITU MID table.
 */
...
const MID_TO_ISO: Record<string, string> = {
  ...
  501: 'FR', 503: 'AU', 506: 'MM', 508: 'BN', 510: 'FM', 511: 'PW', 512: 'NZ', 514: 'KH',
...
export type ShipFlag = { iso: string; emoji: string };

/** Resolve an MMSI to its flag state. Returns null if the MID is unknown. */
export function flagFromMmsi(mmsi: number | string | undefined | null): ShipFlag | null {
  if (mmsi == null) return null;
  const mid = String(mmsi).slice(0, 3);
  const iso = MID_TO_ISO[mid];
  if (!iso) return null;
  return { iso, emoji: isoToEmoji(iso) };
}
